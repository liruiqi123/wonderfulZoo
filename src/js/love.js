...
function printAllLoveAnimalById(animalId){
    App.init();
    ...
    web3.eth.getAccounts(function(error, accounts) {
        ...
        App.contracts.User.deployed().then(function(instance) {
            ...
            if(animalSataus=="2") {
                divA = document.getElementById("loveAnimals");
...
function printYourLoveAnimalById(animalId){
    App.init();
    web3.eth.getAccounts(function(error, accounts) {
        if (error) {
            alert(error);
        ...
        App.contracts.User.deployed().then(function(instance) {
            ...
            if((animalSataus=="2")&&(owner==account)) {
                divA = document.getElementById("yourLoveAnimal");
                divA.innerHTML = divA.innerHTML + '<div class="col-md-4 gallery-grid">'
                    + '<div class="grid">'
                    + '<figure class="effect-apollo">'
                    + '<a class="example-image-link">'
...
function toSeeYourLoveAnimal(){
    App.init();
    var animalById = document.getElementsByName("myselfId");
    document.getElementById('yourLoveAnimal').innerHTML = "";
    for(var i=0;i<animalById.length;i++){
        var id = animalById[i].innerText; //这个值就是你要的
        printYourLoveAnimalById(id);
    }
}
